...
const test = async (accounts) => {
    ...
    for (let i = j; i <j+1; i++) {
       ...
       /* console.log("VP")
        ...
        vcCreationTimes.push([res/config.merkleTree.iterations]);
        console.log("-------------------------------------------------------------------------")
    ...
    file.appendFile('test_VerifyVP_NEW_100.txt', Math.pow(2,j) + ' ' + vcCreationTimes + '\n', function (err) {
        if (err) throw err;
        console.log('Saved!');
    });

   /* const file = fs.createWriteStream('test_VerifyVP_NEW.txt');
